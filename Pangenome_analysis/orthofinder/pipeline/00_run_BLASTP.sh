...
#this script assigns each gene in the Afu293 reference genome to OGs derrived using orthofinder. Here, we use the longest representative sequece in each orthogroup as the input sequnce to make the blast database, and the Af293 reference genes (v. 55) downloaded from fungiDB.
...
#note, we're setting the max_target_seqs to a redic high number here and filtering results afterwards to avoid known problems with the command line version of max_target_seqs ecxluding top hits. 
